...
本稿では、Parallel Key Geometric Flow（PKGF）における統合相（Unified Phase）の作用素進化に付随して現れる **スペクトル流（spectral flow）** の挙動を検討する。
PKGF は、交換子型の保存的成分と楕円型作用素により生成される散逸的成分を複素化によって統合する枠組みである。このとき得られる時間依存作用素族を、固定された楕円型作用素の摂動として実現すると、古典的な自己共役 Fredholm 作用素に対するスペクトル流理論の枠組みに自然に収まる [Atiyah 1967; Doll et al. 2023]。
